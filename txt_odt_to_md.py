...
def is_ai_thought(line: str) -> bool:
    # Viñetas "• " (u+2022) o variantes
    return bool(re.match(r"^\s*[•\-\*\u2022]\s+", line))

def is_question(line: str) -> bool:
    # "? …" o "> PNL …" (palabra natural)
    if re.match(r"^\s*\?\s+", line): return True
    return bool(re.match(r"^\s*>\s*PNL[:\-]?\s+", line, flags=re.IGNORECASE))

def is_prompt(line: str) -> bool:
    # "> …" que NO sea "> PNL …"
    return bool(re.match(r"^\s*>\s+(?!PNL\b)", line, flags=re.IGNORECASE))
...
def convert_lines_to_markdown(lines: List[str], inline_css: bool) -> str:
    ...
    while i<n:
        ...
        # Pensamiento IA (verde cursiva). Agrupa párrafo simple.
        if is_ai_thought(line):
            text=re.sub(r"^\s*[•\-\*\u2022]\s*","",line).strip()
            j=i+1; extra=[]
            while j<n and lines[j].strip() and not is_diff_start(lines[j]) and not looks_like_code(lines[j]) and not is_question(lines[j]) and not is_prompt(lines[j]):
        ...
        # Pregunta (cian)
        if is_question(line):
            q = re.sub(r"^\s*(\?\s+|>\s*PNL[:\-]?\s+)", "", line, flags=re.IGNORECASE).strip()
            i+=1
            out.append(f'<span class="q">{escape_html(q)}</span>')
            continue

        # Prompt genérico (> …) en amarillo. Agrupa líneas consecutivas con '>' también.
        if is_prompt(line):
            block=[]
            while i<n and is_prompt(lines[i]):
                blk = re.sub(r"^\s*>\s+(?!PNL\b)", "", lines[i], flags=re.IGNORECASE).rstrip()
                block.append(blk); i+=1
            text=" ".join(block).strip()
            out.append(f'<span class="prompt">{escape_html(text)}</span>')
            continue

        # Código por heurística
